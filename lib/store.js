...
export class Store {
  ...
  transition(idOrCode, input) {
    ...
    if (to === "待复核") {
      if (item.riggings.length === 0) {
        throw new ApiError(422, "no_riggings", "进入「待复核」前至少需要添加一根帆索");
      }
      const uncalibrated = item.riggings.filter((r) => r.status === "待校准");
      if (uncalibrated.length) {
        throw new ApiError(
          422,
          "uncalibrated_riggings",
          `进入「待复核」前所有帆索必须完成校准，未校准：${uncalibrated.map((r) => r.position).join("、")}`,
          { positions: uncalibrated.map((r) => r.position) }
        );
      }
    }
    if (to === "已交付") {
